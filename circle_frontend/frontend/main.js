...
function openRepostModal(postId) {
  ...
  document.getElementById("modal-orig-text").textContent = post.text || "";
...
async function renderProfile(viewedUserId = null) {
  ...
  if (isOwnProfile) {
    ...
    actionsEl.innerHTML = `<button class="btn ${isFollowing ? "btn-outline" : "btn-primary"}" style="font-size:13px;padding:8px 20px" data-following="${isFollowing}" onclick="toggleFollow(${targetId}, this)">${isFollowing ? "Following" : "Follow"}</button>`;
  }
  ...
  c.innerHTML = sorted.length
    ? sorted.map((p) => buildPostCard(p, isOwnProfile)).join("")
...
function buildPostCard(post, showDelete = false) {
  ...
  return `<div class="post-card" data-post-id="${post.id}">
    ...
    <div class="post-head">
      <div class="av" style="background:${post.authorPicture ? "transparent" : color};cursor:pointer" onclick="viewProfile(${post.userId})">${post.authorPicture ? `<img src="${post.authorPicture}" alt="${escHtml(post.author.charAt(0))}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;display:block"/>` : escHtml(post.author.charAt(0))}</div>
      <div class="post-meta"><div class="post-name clickable" onclick="viewProfile(${post.userId})">${escHtml(post.author)}</div><div class="post-time">${formatTime(post.createdAt)}</div></div>
...
async function handleProfilePicUpload(event) {
  ...
  reader.onload = async (e) => {
    try {
      const res = await api("PUT", `/api/users/${currentUser.id}/picture`, {
  ...
  if (searchTab === "posts") {
    box.innerHTML = data
      .map((post) => {
        ...
        <div class="post-head">
          <div class="av" style="background:${post.authorPicture ? "transparent" : color};cursor:pointer" onclick="viewProfile(${post.userId})">${avHtml}</div>
      ...
      .map((user) => {
        ...
        const followBtnHtml =
          !isOwnProfile && currentUser
            ? `<button class="follow-btn ${user.isFollowing ? "unfollow" : "follow"}" data-following="${user.isFollowing ? "true" : "false"}" onclick="toggleFollow(${user.id}, this)">${user.isFollowing ? "Following" : "Follow"}</button>`
            : "";
        return `<div class="people-card">
        <div class="av" style="background:${user.picture ? "transparent" : color};cursor:pointer" onclick="viewProfile(${user.id})">${avHtml}</div>
        <div class="people-card-info">
          <div class="people-card-name clickable" onclick="viewProfile(${user.id})">${highlight(user.name, q)}</div>

...
} = require('../config/constants12');

// ── Score a single post ────────────────────────────────────
function computeScore(post, viewerUserId, followingIds = []) {
  const hoursOld = (Date.now() - new Date(post.createdAt).getTime()) / 3_600_000;

  // Engagement velocity — likes/comments/reposts divided by age
  // A post with 10 likes in 1h scores higher than 10 likes in 100h
  // This is the YouTube approach: reward content gaining attention fast
  const velocity = (
    (post.likes?.length    || 0) * WEIGHT_LIKE +
    (post.comments?.length || 0) * WEIGHT_COMMENT +
    (post.reposts?.length  || 0) * WEIGHT_REPOST
  ) / Math.max(hoursOld, 1);

  // Soft recency — only a tiebreaker, not the main driver
  // Old posts with high engagement can still surface
  ...
  return velocity + recency + ownBoost + followBoost + repostBoost;
}

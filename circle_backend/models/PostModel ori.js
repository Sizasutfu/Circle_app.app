...
// Returns only top-level comments; each has a `replies` array.
function nestComments(flatComments) {
  ...
  // Second pass: attach replies to their parent, collect roots
  flatComments.forEach(c => {
...
async function hydratePosts(posts) {
  ...
  const [[allLikes], [allReposts], [allComments]] = await Promise.all([
    ...
    db.query(
      // FIX: select parent_id so we can build the reply tree
  ...
  // Group flat rows by post, then nest into tree per post
  ...
  posts.forEach(p => {
    p.likes        = lMap[p.id] || [];
    p.reposts      = rMap[p.id] || [];
    // FIX: nest flat comments into parent → replies tree
    p.comments     = nestComments(cMap[p.id] || []);
    if (p.isRepost && p.originalPostId)
      p.originalPost = origMap[p.originalPostId] || null;

    // Ensure video has a valid data URI prefix so the browser can play it
    if (p.video && !p.video.startsWith('data:')) {
      p.video = `data:video/mp4;base64,${p.video}`;
    }
    if (p.originalPost?.video && !p.originalPost.video.startsWith('data:')) {
      p.originalPost.video = `data:video/mp4;base64,${p.originalPost.video}`;
    }
  });
...
async function createPost(userId, text, image, video) {
  try {
    ...
    console.log("is string:", typeof video === String);
    console.log("preview:", video?.substring(0, 50));
...
// FIX: accept optional parentId to store the reply relationship
async function addComment(postId, userId, text, parentId = null) {
  // If a parentId is given, verify it belongs to the same post
  // to prevent attaching replies to comments on other posts.
  if (parentId) {

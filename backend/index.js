...
const ok  = (res, status, msg, data = null) =>
  res.status(status).json({ success: true,  message: msg, ...(data !== null && { data }) });
...
async function hydratePosts(posts) {
  ...
  const [[allLikes], [allReposts], [allComments]] = await Promise.all([
    ...
    db.query(
      `SELECT c.id, c.post_id, c.user_id AS userId,
       ...
       WHERE c.post_id IN (${ph}) ORDER BY c.created_at ASC`, ids
    ),
  ]);
  ...
  const origIds = [...new Set(posts.filter(p => p.isRepost && p.originalPostId).map(p => p.originalPostId))];
  let origMap = {};
  if (origIds.length) {
    ...
    const [origRows] = await db.query(
      `SELECT p.id, u.name AS author, u.picture AS authorPicture, p.text, p.image
       FROM posts p JOIN users u ON u.id = p.user_id WHERE p.id IN (${oph})`, origIds
    );
...
app.post('/api/users/register', async (req, res) => {
  ...
  try {
    ...
    return ok(res, 201, 'Registered successfully.', { id: r.insertId, name, email, picture: null, createdAt: new Date() });
...
app.post('/api/users/login', async (req, res) => {
  ...
  try {
    ...
    const user = rows[0];
    if (!await bcrypt.compare(password, user.password)) return err(res, 401, 'Wrong password.');
...
app.get('/api/users/:id/profile', async (req, res) => {
  ...
  try {
    const [rows] = await db.query('SELECT id, name, email, picture FROM users WHERE id=?', [userId]);
    if (!rows.length) return err(res, 404, 'User not found.');

    const [[{ postCount }]]      = await db.query(`SELECT COUNT(*) AS postCount      FROM posts   WHERE user_id=? AND is_repost=0`, [userId]);
    const [[{ followerCount }]]  = await db.query(`SELECT COUNT(*) AS followerCount  FROM follows WHERE following_id=?`,           [userId]);
    const [[{ followingCount }]] = await db.query(`SELECT COUNT(*) AS followingCount FROM follows WHERE follower_id=?`,            [userId]);

    // Is the viewer currently following this profile?
    let isFollowing = false;
    if (viewerId && viewerId !== userId) {
      const [f] = await db.query('SELECT id FROM follows WHERE follower_id=? AND following_id=?', [viewerId, userId]);
...
app.put('/api/users/:id/picture', requireAuth, async (req, res) => {
  ...
  if (picture && picture.length > 7_000_000) return err(res, 413, 'Image too large (max 5 MB).');
  try {
...
app.put('/api/users/:id', requireAuth, async (req, res) => {
  ...
  try {
    const [conflict] = await db.query('SELECT id FROM users WHERE email=? AND id!=?', [email, userId]);
    ...
    const [u] = await db.query('SELECT id,name,email,picture,created_at AS createdAt FROM users WHERE id=?', [userId]);
...
app.post('/api/follow/:targetId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [target] = await db.query('SELECT id, name FROM users WHERE id=?', [followingId]);
    if (!target.length) return err(res, 404, 'User not found.');

    // Already following?
    const [existing] = await db.query(
      'SELECT id FROM follows WHERE follower_id=? AND following_id=?', [followerId, followingId]
    );
    ...
    const [[{ followerCount }]] = await db.query(
      'SELECT COUNT(*) AS followerCount FROM follows WHERE following_id=?', [followingId]
    );

    // Notify the followed user
    await createNotification(followingId, followerId, 'follow', null);

    return ok(res, 201, `You are now following ${target[0].name}.`, { followerCount });
  } catch (e) { console.error(e); return err(res, 500, 'Server error.'); }
});
...
app.delete('/api/unfollow/:targetId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!existing.length) return err(res, 404, 'You are not following this user.');
...
app.get('/api/followers/:userId', async (req, res) => {
  ...
  try {
    ...
    if (viewerId) {
      ...
      rows.forEach(u => { u.isFollowing = set.has(u.id); });
    }
  ...
  try {
    ...
    if (viewerUserId) {
      const [fRows] = await db.query(
        'SELECT following_id FROM follows WHERE follower_id=?', [viewerUserId]
      );
    ...
    if (feedMode === 'following' && viewerUserId) {
      if (!followingIds.length) {
        ...
        return ok(res, 200, 'Posts fetched.', { posts: [], hasMore: false, page });
      }
...
app.post('/api/posts', requireAuth, async (req, res) => {
  ...
  try {
    const [u] = await db.query('SELECT id, name FROM users WHERE id=?', [userId]);
    if (!u.length) return err(res, 404, 'User not found.');
    const [r] = await db.query('INSERT INTO posts (user_id, text, image) VALUES (?,?,?)', [userId, text||null, image||null]);
    return ok(res, 201, 'Posted.', {
      id: r.insertId, userId, author: u[0].name,
...
app.post('/api/posts/:id/comment', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [u] = await db.query('SELECT id, name, picture FROM users WHERE id=?', [userId]);
    if (!u.length) return err(res, 404, 'User not found.');
    const [r] = await db.query('INSERT INTO comments (post_id, user_id, text) VALUES (?,?,?)', [postId, userId, text]);
    await createNotification(post[0].user_id, userId, 'comment', postId);
    return ok(res, 201, 'Comment added.', {
      id: r.insertId, userId,
      author: u[0].name, authorPicture: u[0].picture || null,
...
app.post('/api/posts/:id/like', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (existing.length) {
      ...
      const [[{ total }]]    = await db.query('SELECT COUNT(*) AS total FROM likes WHERE post_id=?', [postId]);
      const [[owner]]        = await db.query('SELECT user_id FROM posts WHERE id=?', [postId]);
      if (owner) await createNotification(owner.user_id, userId, 'like', postId);
...
app.post('/api/posts/:id/repost', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [u]    = await db.query('SELECT id, name FROM users WHERE id=?', [userId]);
    if (!u.length) return err(res, 404, 'User not found.');
    const [dup]  = await db.query('SELECT id FROM reposts WHERE user_id=? AND original_post_id=?', [userId, origId]);
    ...
    await db.query('INSERT INTO reposts (user_id, original_post_id, repost_post_id) VALUES (?,?,?)',
      [userId, origId, pr.insertId]);

    const [origInfo] = await db.query(
      `SELECT p.id, u.name AS author, u.picture AS authorPicture, p.text, p.image
       FROM posts p JOIN users u ON u.id=p.user_id WHERE p.id=?`, [origId]
    );
    await createNotification(orig[0].user_id, userId, 'repost', origId);

    return ok(res, 201, 'Reposted.', {
      id: pr.insertId, userId, author: u[0].name,
      text: text||'', image: null,
      isRepost: true, originalPostId: origId,
...
app.get('/api/notifications/:userId/unread-count', async (req, res) => {
  ...
  try {
    const [[{ count }]] = await db.query(
      'SELECT COUNT(*) AS count FROM notifications WHERE recipient_id=? AND is_read=0', [userId]
    );
...
app.put('/api/notifications/:userId/read-all', async (req, res) => {
  const userId = parseInt(req.params.userId);
  try {
    await db.query('UPDATE notifications SET is_read=1 WHERE recipient_id=?', [userId]);
...
app.get('/api/search', async (req, res) => {
  ...
  if (q.length < 2) return err(res, 400, 'Query must be at least 2 characters.');
  const like = `%${q}%`;
  try {
    if (type === 'people') {
      const viewerId = parseInt(req.headers['x-user-id']) || null;
      ...
      // Tag follow status
      if (viewerId) {
        const [fRows] = await db.query('SELECT following_id FROM follows WHERE follower_id=?', [viewerId]);
        const set     = new Set(fRows.map(r => r.following_id));
        users.forEach(u => { u.isFollowing = set.has(u.id); });
      }
...
app.get('/', (req, res) => {
  res.json({
    ...
    features: ['pagination (15/page)','follow system','follow-feed','auth middleware','notifications'],
  });
});

app.use((req, res) => err(res, 404, `Route '${req.originalUrl}' not found.`));

app.listen(PORT, () => console.log(`✅  Circle API running on http://localhost:${PORT}`));